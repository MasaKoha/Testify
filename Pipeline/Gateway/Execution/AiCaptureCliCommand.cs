...
namespace UniTestify.Pipeline
{
    ...
    {
        /// <summary>撮影を要求します。view の適用成功時はフォーカスだけを行い、次回に撮影します。</summary>
        ...
        public static string Capture(
            ...
            [CliArg("view", "game / simulator。適用成功時はフォーカスのみ。次回は省略して撮影。")] string view = "")
        {
            var response = AiCommandDispatcher.Execute(new AiCommandRequest
            {
                op = "capture",
                args = JsonUtility.ToJson(new AiCliArguments { name = name, directory = directory, view = view }),
            });
