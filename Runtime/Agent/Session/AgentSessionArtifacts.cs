    ...
    internal sealed class AgentSessionArtifacts
    {
        ...
        internal void AppendActionLog(AgentAction action, string observationKey, string actionKind, string target, string status, string message, string diffText)
        {
            var entry = new AgentActionLogEntry
            {
                ...
                waitForText = action?.waitForText ?? string.Empty,
                waitForObject = action?.waitForObject ?? string.Empty,
                waitForFocus = action?.waitForFocus ?? string.Empty,
                waitForScene = action?.waitForScene ?? string.Empty,
                timeoutSeconds = action?.timeoutSeconds ?? UiScenarioStep.DefaultTimeoutSeconds,
